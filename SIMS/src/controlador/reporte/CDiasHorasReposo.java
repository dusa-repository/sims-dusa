...
public class CDiasHorasReposo extends CGenerico {
	...
	@Wire
	private Div botoneraReposos;
	@Wire
	private Div divReposos;
	...
	public void inicializar() throws IOException {
		...
		Botonera botonera = new Botonera() {
			...
			@Override
			public void salir() {
				cerrarVentana(divReposos, nombre, tabs);
			}
			...
			public void guardar() {
				...
				List<Consulta> reposos= servicioConsulta.buscarEntreFechasReposoyTrabajadores(desde, hasta);
				if (!reposos.isEmpty())
					Clients.evalJavaScript("window.open('"
							+ damePath()
							+ "Reportero?valor=61&valor6="
							+ fecha1
							+ "&valor7="
							+ fecha2
							+ "&valor20="
		...
		botoneraReposos.appendChild(botonera);
	}
	...
	public byte[] reporteResumen(String par6, String par7,
		...
		try {
			reporte = (JasperReport) JRLoader.loadObject(getClass()
					.getResource("/reporte/RDiasReposo.jasper"));
		} catch (JRException e) {
			msj = new Mensaje();
		...
		if (tipo2.equals("EXCEL")) {
			...
			try {
				jasperPrint = JasperFillManager.fillReport(reporte, p,
						new JRBeanCollectionDataSource(reposos));
			} catch (JRException e) {
