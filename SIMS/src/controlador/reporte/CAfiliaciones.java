...
public class CAfiliaciones extends CGenerico {
	...
	@Wire
	private Div botoneraAfiliaciones;
	@Wire
	private Div divAfiliaciones;
	...
	public void inicializar() throws IOException {
		...
		Botonera botonera = new Botonera() {
			...
			@Override
			public void salir() {
				cerrarVentana(divAfiliaciones, nombre, tabs);
			}
			...
			public void guardar() {
				...
				List<Paciente> afiliaciones= servicioPaciente.buscarEntreFechasAfiliaciones(desde, hasta);
				if (!afiliaciones.isEmpty())
					Clients.evalJavaScript("window.open('"
							+ damePath()
							+ "Reportero?valor=60&valor6="
							+ fecha1
							+ "&valor7="
							+ fecha2
							+ "&valor20="
		...
		botoneraAfiliaciones.appendChild(botonera);
	}
	...
	public byte[] reporteResumen(String par6, String par7,
		...
		List<Paciente> pacientes = getServicioPaciente()
				.buscarEntreFechasAfiliaciones(fecha1, fecha2);
		...
		try {
			reporte = (JasperReport) JRLoader.loadObject(getClass()
					.getResource("/reporte/RAfiliaciones.jasper"));
		} catch (JRException e) {
			msj = new Mensaje();
